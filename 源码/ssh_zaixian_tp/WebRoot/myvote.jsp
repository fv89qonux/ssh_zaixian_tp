...
<script>
    $(function () {
        $("#myvote").addClass("active");
        $.ajax({
            type: "POST", //用POST方式传输
            dataType: "text", //数据格式:JSON
            url: "findVotedSubjectByUserId", //目标地址
            data: {
                userId: "<%=user.getId()%>",
            },
            ...
            success: function (msg) {
                ...
                var html = "";
                for (var name in map) {
                    html+="<div class='dd-handle'><h4>投票："+map[name].map.subjectName+"&nbsp;&nbsp;&nbsp;所投选项："+(function(){
                                var op="";
                                for (var o in map[name].map.optionInfo.map){
                                   op+=map[name].map.optionInfo.map[o] +";  "
                                }
                                return op;
                            })()+"</h4></div>"
                }
                if(html ==""){
                    html="<div class='dd-handle'><h4>暂时没有投票记录</h4></div>"
                }
                $("#target").append(html);
